...
ShadNetConnection::ShadNetConnection() { platformInit(); }

ShadNetConnection::~ShadNetConnection() { disconnect(); }
...
bool ShadNetConnection::connect(const char* host, uint16_t port)
{
	...
	if (inet_pton(AF_INET, host, &addr.sin_addr) <= 0) {
		...
		SOCK_CLOSE(m_sock); m_sock = INVALID_SOCK;
		return false;
	}
...
bool ShadNetConnection::recvAll(uint8_t* buf, int n)
{
	...
	while (got < n) {
		int r = static_cast<int>(
			::recv(m_sock, reinterpret_cast<char*>(buf + got), n - got, 0));
		if (r <= 0) return false;
		got += r;
	}
	return true;
	...
	while (sent < total) {
		...
		int r = static_cast<int>(
			::send(m_sock,
				...
				total - sent, 0));
		if (r < 0) { m_lastError = "send() failed"; return false; }
		sent += r;
	}
	return true;
}
...
void ShadNetConnection::parse()
{
	while (m_readBuf.size() >= HEADER_SIZE)
	{
		...
		m_readBuf.erase(m_readBuf.begin(),
			m_readBuf.begin() + static_cast<int>(totalSize));
