...
{
	if (!conn.connect(host, port)) return false;
	...
	const std::string& password, const std::string& token)
{
	...
	addStr(npid); addStr(password); addStr(token);
	...
	const std::string& email)
{
	...
	addStr(npid); addStr(password); addStr(onlineName); addStr(avatarUrl); addStr(email);
...
void ShadNetClient::addFriend(const std::string& n) { sendFriendCommand(CommandType::AddFriend, n); }
void ShadNetClient::removeFriend(const std::string& n) { sendFriendCommand(CommandType::RemoveFriend, n); }
void ShadNetClient::addBlock(const std::string& n) { sendFriendCommand(CommandType::AddBlock, n); }
void ShadNetClient::removeBlock(const std::string& n) { sendFriendCommand(CommandType::RemoveBlock, n); }

// NPScore wire helpers

// Build the standard score request payload:
//   ComId(12 bytes, padded) + u32 LE blob size + serialised protobuf
static std::vector<uint8_t> makeScorePayload(
	const std::string& comId, const google::protobuf::MessageLite& msg)
{
...
{
	if (pos + 4 > static_cast<int>(p.size())) return {};
	uint32_t len = 0; memcpy(&len, p.data() + pos, 4); pos += 4;
	if (pos + static_cast<int>(len) > static_cast<int>(p.size())) return {};
	...
	const std::string& comment, const std::vector<uint8_t>& gameInfo)
{
	...
	req.set_pcid(pcId);
	req.set_score(score);
	if (!comment.empty())  req.set_comment(comment);
	if (!gameInfo.empty()) req.set_data(gameInfo.data(), gameInfo.size());
	conn.send(buildPacket(CommandType::RecordScore, packetCounter++,
		makeScorePayload(comId, req)));
}

void ShadNetClient::recordScoreData(const std::string& comId, uint32_t boardId,
	int32_t pcId, int64_t score, const std::vector<uint8_t>& data)
{
	...
	const std::string& npid, int32_t pcId)
{
	...
	req.set_npid(npid);
	req.set_pcid(pcId);
	conn.send(buildPacket(CommandType::GetScoreData, packetCounter++,
		makeScorePayload(comId, req)));
}

void ShadNetClient::getScoreRange(const std::string& comId, uint32_t boardId,
	uint32_t startRank, uint32_t numRanks, bool withComment, bool withGameInfo)
{
	...
	conn.send(buildPacket(CommandType::GetScoreRange, packetCounter++,
		makeScorePayload(comId, req)));
}

void ShadNetClient::getScoreNpid(const std::string& comId, uint32_t boardId,
	const std::vector<NpIdPcId>& targets, bool withComment, bool withGameInfo)
{
	...
	req.set_withcomment(withComment);
	req.set_withgameinfo(withGameInfo);
	conn.send(buildPacket(CommandType::GetScoreNpid, packetCounter++,
		makeScorePayload(comId, req)));
}

void ShadNetClient::getScoreFriends(const std::string& comId, uint32_t boardId,
	bool includeSelf, uint32_t max, bool withComment, bool withGameInfo)
{
	...
	req.set_withcomment(withComment);
	req.set_withgameinfo(withGameInfo);
	conn.send(buildPacket(CommandType::GetScoreFriends, packetCounter++,
		makeScorePayload(comId, req)));
}
...
{
	if (pos + 12 > static_cast<int>(p.size())) return false;
	pos += 12;
	for (int i = 0; i < 3; ++i) {
		while (pos < static_cast<int>(p.size()) && p[pos] != 0) ++pos;
		if (pos >= static_cast<int>(p.size())) return false;
		++pos;
	}
	if (pos + 4 > static_cast<int>(p.size())) return false;
...
void ShadNetClient::handleLoginReply(const std::vector<uint8_t>& payload)
{
	...
	if (payload.empty()) { res.error = ErrorType::Malformed; }
		...
		if (res.error == ErrorType::NoError) {
			...
			auto readStr = [&]() -> std::string {
				...
				if (pos < static_cast<int>(payload.size())) ++pos;
				return s;
				};
			auto readU32 = [&]() -> uint32_t {
				if (pos + 4 > static_cast<int>(payload.size())) return 0;
				uint32_t v = fromLE32(payload.data() + pos); pos += 4; return v;
				};
			res.onlineName = readStr();
			res.avatarUrl = readStr();
			if (pos + 8 <= static_cast<int>(payload.size())) {
				res.userId = fromLE64(payload.data() + pos); pos += 8;
			}
			uint32_t fc = readU32();
	...
	if (res.error == ErrorType::NoError) {
		printf("[login] OK  onlineName=%s  userId=%llu\n",
			res.onlineName.c_str(), static_cast<unsigned long long>(res.userId));
...
void ShadNetClient::handleNotification(const Packet& pkt)
{
	...
	auto readStr = [&]() -> std::string {
		...
		if (pos < static_cast<int>(p.size())) ++pos;
		return s;
		};
	...
	case NotificationType::FriendQuery: {
		NotifyFriendQuery n; n.fromNpid = readStr();
		printf("[notify] FriendQuery from %s\n", n.fromNpid.c_str());
		if (onFriendQuery) onFriendQuery(n); break;
	}
	case NotificationType::FriendNew: {
		NotifyFriendNew n;
		if (pos < static_cast<int>(p.size())) n.online = (p[pos++] != 0);
		n.npid = readStr();
		printf("[notify] FriendNew %s (%s)\n", n.npid.c_str(), n.online ? "online" : "offline");
		if (onFriendNew) onFriendNew(n); break;
	}
	case NotificationType::FriendLost: {
		NotifyFriendLost n; n.npid = readStr();
		printf("[notify] FriendLost %s\n", n.npid.c_str());
		if (onFriendLost) onFriendLost(n); break;
	}
	case NotificationType::FriendStatus: {
		NotifyFriendStatus n;
		if (pos < static_cast<int>(p.size())) n.online = (p[pos++] != 0);
		if (pos + 8 <= static_cast<int>(p.size())) { n.timestamp = fromLE64(p.data() + pos); pos += 8; }
		n.npid = readStr();
		printf("[notify] FriendStatus %s is %s\n", n.npid.c_str(), n.online ? "online" : "offline");
		if (onFriendStatus) onFriendStatus(n); break;
	}
	default:
...
{
	if (pkt.type == PacketType::Notify) { handleNotification(pkt); return; }
	...
	case CommandType::RemoveBlock:
		handleFriendReply(static_cast<CommandType>(pkt.command), pkt.payload);  break;
	case CommandType::GetBoardInfos:   handleBoardInfosReply(pkt.payload);      break;
...
{
	BoardInfo info;
	if (payload.empty()) { if (onBoardInfos) onBoardInfos(info); return; }
	...
	if (info.error == ErrorType::NoError) {
		...
		printf("[board-info] rankLimit=%u updateMode=%u sortMode=%u"
			...
			info.rankLimit, info.updateMode, info.sortMode,
			info.uploadNumLimit, info.uploadSizeLimit);
	}
	else {
		printf("[board-info] FAILED: %s\n", errorName(info.error));
	}
	if (onBoardInfos) onBoardInfos(info);
}

void ShadNetClient::handleRecordScoreReply(const std::vector<uint8_t>& payload)
{
	RecordScoreResult res;
	if (payload.empty()) { if (onRecordScore) onRecordScore(res); return; }
	res.error = static_cast<ErrorType>(payload[0]);
	...
	if (onRecordScore) onRecordScore(res);
}
...
void ShadNetClient::handleRecordScoreDataReply(const std::vector<uint8_t>& payload)
{
	ErrorType err = payload.empty() ? ErrorType::Malformed
		: static_cast<ErrorType>(payload[0]);
	if (err == ErrorType::NoError) printf("[record-score-data] OK\n");
	else                           printf("[record-score-data] FAILED: %s\n", errorName(err));
	if (onRecordScoreData) onRecordScoreData(err);
}
...
void ShadNetClient::handleGetScoreDataReply(const std::vector<uint8_t>& payload)
{
	...
	if (err == ErrorType::NoError && payload.size() >= 5) {
		uint32_t blobLen = 0; memcpy(&blobLen, payload.data() + 1, 4);
	...
	std::function<void(const ScoreRangeResult&)>& cb)
{
	...
	for (int i = 0; i < pb.rankarray_size(); ++i) {
		...
		if (i < pb.infoarray_size())
			e.gameInfo = std::vector<uint8_t>(
				pb.infoarray(i).data().begin(),
				pb.infoarray(i).data().end());
