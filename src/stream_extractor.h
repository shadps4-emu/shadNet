...
public:
	explicit StreamExtractor(const QByteArray& data)
		: m_data(data), m_pos(0), m_error(false) {
	}
	...
	QString getString(bool empty = false) {
		...
		++m_pos; // consume null
		if (m_pos > m_data.size() || (!empty && s.isEmpty()))
			m_error = true;
		return s;
	}
	...
	QByteArray getRawData() {
		...
		if (m_error) return {};
		if (m_pos + (int)size > m_data.size()) { m_error = true; return {}; }
...
private:
	template<typename T> static T fromLe(T v) {
