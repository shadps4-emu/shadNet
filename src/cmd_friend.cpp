...
// Build the friendlist:friend extended-data pairs per the SDK: trigger names the updated
// friend's onlineId + add/remove event; additionalTrigger names their accountId + event.
static QList<QPair<QString, QString>> FriendExtd(const QString& friendNpid,
                                                 int64_t friendAccountId, const char* event) {
