...
struct SharedState {
    ...
    // PSN Session Manager (Session/Invitation Web API). Sessions are created+joined via
    // POST /v1/sessions and live here keyed by sessionId ("001-<uuid>"). Protected by the
    // dedicated sessionsLock (never nested with clientsLock/usageLock).
    ...
    QHash<QString, Session> sessions; // protected by sessionsLock
};
