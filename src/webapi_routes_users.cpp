...
constexpr quint32 UP_INVALID_QUERY_PARAM = 2105601;     // bad value in query string
constexpr quint32 UP_QUERY_PARAM_REQUIRED = 2105605;    // required query param missing
constexpr quint32 UP_ACCESS_DENIED_OWNERSHIP = 2105358; // non-owner accessed the resource
...
constexpr char DefaultRegion[] = "us";

QHttpServerResponse JsonError(QHttpServerResponse::StatusCode status, quint32 code,
...
void ParsePaging(const QUrlQuery& query, int fallbackDefault, int maxLimit, int& limit,
                 int& offset) {
    bool ok = false;
